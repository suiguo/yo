...
func GetLogger(tag string) WarpLog {
	...
	tmp := &logger{logger: zapLogger.Sugar(), zap_base: zapLogger, atomicLevels: []*zap.AtomicLevel{&atomicLevel}}
	loggerMap.Store(tag, tmp)
	return tmp
}
...
func getWriter(cfg *LoggerCfg) zapcore.WriteSyncer {
	...
	for _, path := range cfg.WriterFilePath {
		...
		} else if strings.EqualFold(path, "/dev/null") {
			writers = append(writers, zapcore.AddSync(io.Discard))
		} else {
